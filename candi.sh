...
#############################################################
# Required packages
PACKAGES="openmpi p4est kokkos zlib"
...
# Check that the version is greater than 9.5
if ! version_greater_equal "$DEAL_II_VERSION" "9.5"; then
	color_echo ${BAD} "Invalid value for DEAL_II_VERSION=$DEAL_II_VERSION. Expected version greater than 9.5"
	exit 1
fi

# Check that multiple spack options aren't selected
...
#############################################################
# Update the dependencies for PRISMS-PF and PRISMS-Plasticity
if [ "$PRISMS_PLASTICITY" == "ON" ]; then
	PACKAGES="${PACKAGES} hdf5 petsc"
fi
if [ "$PRISMS_PF" == "ON" ]; then
...
sort_packages() {
	...
	color_echo ${INFO} "Preparing to install ${PACKAGES} as dependencies"
...
if [ "$USE_SPACK_FOR_COMPILER" == "ON" ]; then
